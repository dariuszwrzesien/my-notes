...
# 2. Uruchomienie wszystkich aplikacji jedną komendą
...
**Gotowe!** Wszystkie 4 aplikacje uruchomią się automatycznie używając `concurrently`.

### Opcja 2: Tryb preview (production build) 📦

```bash
# 1. Instalacja zależności (jeśli nie było)
./install-all.sh

# 2. Build i uruchomienie w trybie production
...
- ✅ Buduje wszystkie aplikacje (`npm run build`)
- ✅ Uruchamia serwery preview (`npm run preview`)
...
**WAŻNE**: Kolejność uruchamiania ma znaczenie! Design System musi być dostępny przed MFE.
...
- 🏠 **Host (główna)**: http://localhost:5000 ← **Otwórz ten!**
